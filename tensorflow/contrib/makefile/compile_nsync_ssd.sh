...
for arch in $archs; do
        ...
        ios)    xcode=/Applications/Xcode.app/Contents/Developer/Platforms
                ...
                i386|x86_64)
                        ...
                        arch_flags="$arch_flags -isysroot $IPHONESIMULATOR_SYSROOT"
                        ;;
                *)
                        arch_flags="$arch_flags -miphoneos-version-min=8.0"
                        arch_flags="$arch_flags -isysroot $IPHONEOS_SYSROOT"
                        ;;
                esac
                makefile='
                        CC=${CC_PREFIX} clang++
...
ios)    nsync_platform_dir="$nsync_builds_dir/lipo.$target_platform.c++11"
        if [ -d "$nsync_platform_dir" ]; then
            rm -r "$nsync_platform_dir"
		fi
        mkdir "$nsync_platform_dir"
        eval lipo $platform_libs -create -output '$nsync_platform_dir/nsync.a'
        if [ -d "$nsync_platform_dir/../../../../gen/nsync" ]; then
            rm -r "$nsync_platform_dir/../../../../gen/nsync"
		fi
        mkdir "$nsync_platform_dir/../../../../gen/nsync"
		cp "$nsync_platform_dir/nsync.a" "$nsync_platform_dir/../../../../gen/nsync/libnsync.a"
		echo "$nsync_platform_dir/nsync.a"
        ;;
esac
